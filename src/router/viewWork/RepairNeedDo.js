...
class WorkOrderNeedDo extends Component {
  ...
  componentDidMount(){
    ...
    AsyncStorage.getItem("main",(error,text)=>{
      if(!error){
        ...
        getNoAssignWorkOrder({currentUserId:data.id,currentPage: 1,pageSize: 10,projectId:_state.projectId,},_this.updata);
      }else{
      }
    });
  }
  componentWillReceiveProps(newProps){
    if(newProps.projectId != _this.props.projectId){
      _this.setState({onSearchLoad: true,reset: true});
      getNoAssignWorkOrder({currentUserId:_state.userId,currentPage: 1,pageSize: 10,projectId:newProps.projectId,},_this.updata);
    }
  }
  updata(result){
  ...
  onSearch(){
    if(!_state.load){
      _this.setState({onSearchLoad: true,reset: true});
      getNoAssignWorkOrder({currentUserId:_state.userId,currentPage: 1,pageSize: 10,projectId:_state.projectId,},_this.updata);
    }
  }
  onEndReached(){
    if(_state.current*_state.size<_state.total && !_state.load){
      _this.setState({load: true,});
      getNoAssignWorkOrder({currentUserId:_state.userId,currentPage: _state.current+1,pageSize: _state.size,projectId:_state.projectId,},_this.updata);
    }
  }
