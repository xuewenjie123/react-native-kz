...
var { getNoAssignWorkOrder, } = require('../../service/workOrder');
...
class WorkOrderNeedDo extends Component {
  constructor(props) {
    super(props);
    this.state = {
      userId: undefined,
  ...
  componentDidMount(){
    ...
    AsyncStorage.getItem("main",(error,text)=>{
      if(!error){
        ...
        getNoAssignWorkOrder({currentUserId:data.id,currentPage: 1,pageSize: 10,},_this.updata);
      }else{
        console.log(error)
      }
    });
  ...
  onSearch(){
    if(!_state.load){
      _this.setState({onSearchLoad: true,reset: true});
      getNoAssignWorkOrder({currentUserId:_state.userId,currentPage: 1,pageSize: 10,},_this.updata);
    }
  }
  onEndReached(){
    if(_state.current*_state.size<_state.total && !_state.load){
      _this.setState({load: true,});
      getNoAssignWorkOrder({currentUserId:_state.userId,currentPage: _state.current+1,pageSize: _state.size,},_this.updata);
    }
  }
...
module.exports = WorkOrderNeedDo
