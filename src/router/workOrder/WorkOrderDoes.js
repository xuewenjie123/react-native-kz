...
class WorkOrderDoes extends Component {
  ...
  componentDidMount(){
    ...
    AsyncStorage.getItem("main",(error,text)=>{
      if(!error){
        ...
        getFinishWorkOrder({currentUserId:data.id,currentPage: 1,pageSize: 10,},_this.updata);
      }else{
        console.log(error)
      }
    });
  ...
  onSearch(){
    if(!_state.load){
      _this.setState({onSearchLoad: true,reset: true});
      getFinishWorkOrder({currentUserId:_state.userId,currentPage: 1,pageSize: 10,},_this.updata);
    }
  }
  onEndReached(){
    if(_state.current*_state.size<_state.total && !_state.load){
      _this.setState({load: true,});
      getFinishWorkOrder({currentUserId:_state.userId,currentPage: _state.current+1,pageSize: _state.size,},_this.updata);
    }
  }
