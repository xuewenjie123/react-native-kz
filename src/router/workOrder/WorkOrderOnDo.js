...
var { getMaintenanceWorkOrder, } = require('../../service/workOrder');
...
class WorkOrderOnDo extends Component {
  constructor(props){
    super(props);
    this.state = {
      userId: undefined,
  ...
  componentDidMount(){
    ...
    AsyncStorage.getItem("main",(error,text)=>{
      if(!error){
        ...
        getMaintenanceWorkOrder({currentUserId:data.id,currentPage: 1,pageSize: 10,},_this.updata);
      }else{
        console.log(error)
      }
    });
  ...
  onSearch(){
    if(!_state.load){
      _this.setState({onSearchLoad: true,reset: true});
      getMaintenanceWorkOrder({currentUserId:_state.userId,currentPage: 1,pageSize: 10,},_this.updata);
    }
  }
  onEndReached(){
    if(_state.current*_state.size<_state.total && !_state.load){
      _this.setState({load: true,});
      getMaintenanceWorkOrder({currentUserId:_state.userId,currentPage: _state.current+1,pageSize: _state.size,},_this.updata);
    }
  }
          ...
          <View style={styles.databox3}>
            <TouchableOpacity onPress={()=>{_this._onPushRouterDetail(rowData)}}>
              ...
              <View style={styles.dabox4}>
                ...
                {rowData.workOrderStatusRecord?(
                  <View style={styles.shedbox}>
                    <View style={styles.shedbox2}>
                      <Text numberOfLines={2} style={styles.text}>
                        {rowData.workOrderStatusRecord.operateUserName+"："}
                        <Text style={styles.text2}>
                          {rowData.workOrderStatusRecord.description}
                        </Text>
                      </Text>
                    </View>
                    {rowData.workOrderStatusRecord.fileList&&rowData.workOrderStatusRecord.fileList.length?<View style={styles.shedbox2}>
                      <Image style={styles.image24} source={{uri: rowData.workOrderStatusRecord.fileList[0]}}></Image>
                    </View>:null}
                  </View>
                ):null}
              </View>
